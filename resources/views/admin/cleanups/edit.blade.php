                    ...
                    <form action="{{ route('admin.cleanups.update', $cleanup->id) }}" method="post" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="location" id="location"
                                value="{{ old('location') ?: $cleanup->location }}"
                        ...
                        <div>
                            ...
                            <input type="time" name="time" id="time"
                                value="{{ old('time') ? old('time') : \Carbon\Carbon::parse($cleanup->time)->format('H:i') }}"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" />
                            @if ($errors->has('time'))
                        ...
                        <div>
                            ...
                            <input type="date" name="date" id="date" value="{{ old('date') ?: $cleanup->date }}"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" />
                            @if ($errors->has('date'))
                        ...
                        <div>
                            ...
                            <input type="text" name="description" id="description"
                                value="{{ old('description') ?: $cleanup->description }}"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" />
                            ...
                            <select name="group_id" id="group"
                                ...
                                @foreach ($groups as $group)
                                    <option {{ old('group_id') == $group->id ? 'selected' : '' }}
                                        value="{{ $group->id }}">{{ $group->location }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="latitude" id="latitude"
                                value="{{ old('latitude') ?: $cleanup->latitude }}"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" />
                        ...
                        <div>
                            ...
                            <input type="text" name="longitude" id="longitude"
                                value="{{ old('longitude') ?: $cleanup->longitude }}"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" />
                        ...
                        <div class="flex justify-between">
                            <button type="submit"
                                class="bg-blue-700 text-white font-bold py-2 px-4 rounded">Update</button>
                            <a href="{{ url()->previous() }}"
                                class="bg-gray-500 text-white font-bold py-2 px-4 rounded">Back</a>
                        </div>
                    </form>
                </div>
            </div>
